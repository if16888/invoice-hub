...
def replace_once(path: Path, old: str, new: str) -> None:
    ...
    newline = "\r\n" if "\r\n" in text else "\n"
    old_disk = _adapt_newlines(old, newline)
    new_disk = _adapt_newlines(new, newline)
    count = text.count(old_disk)
    if count != 1:
        raise RuntimeError(f"{path}: expected exactly one replacement, found {count}: {old[:120]!r}")
    _write_exact(path, text.replace(old_disk, new_disk, 1))


def regex_replace_once(path: Path, pattern: str, replacement: str) -> None:
    text = _read_exact(path)
    if "\r\n" in text:
        raise RuntimeError(f"{path}: regex replacement requires LF file")
...
# 4) Remove the stale hard-coded 3-month UI statement; mailbox rows render the actual value.
replace_once(
    settings,
...
# 5) Document bounded browser policy in the example config.
replace_once(
    config_example,
