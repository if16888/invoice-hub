"""Run native accessibility-state captures in isolated Qt processes."""
...
CAPTURE = ROOT / "scripts" / "dev" / "capture_accessibility_states.py"
STATES = (
    "button-focus",
    "nav-focus",
    "stat-focus",
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / ".codex-artifacts" / "design-v1" / "accessibility-screenshots",
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=ROOT / ".codex-artifacts" / "design-v1" / "accessibility-report.json",
    )
    args = parser.parse_args()

    args.output_dir.mkdir(parents=True, exist_ok=True)
    args.report.parent.mkdir(parents=True, exist_ok=True)
    args.report.write_text(json.dumps({"runs": [], "expected_case_keys": []}, indent=2), encoding="utf-8")
    ...
    for state, width, height, scale in CASES:
        ...
        if result.returncode or not output.exists() or output.stat().st_size == 0:
            raise SystemExit(f"accessibility capture failed: {key}")
