...
from .workbench_state import IncrementalWindow, is_keyboard_input_target
...
class InvoiceReviewApp(PreviewMixin, LogDiagnosticsMixin, QMainWindow):
    ...
    def _init_ui(self):
        ...
        self.lbl_record_summary = QLabel("当前筛选 0 张")
        self.lbl_record_summary.setFont(QFont("Segoe UI", 9))
        self.lbl_record_summary.setStyleSheet("color: #4B5563;")
        self.lbl_record_summary.setToolTip("发票数据统计与选中合计")
        self.lbl_record_summary.setMinimumWidth(300)
        status_layout.addWidget(self.lbl_record_summary, 1)

        # Compatibility aliases
        self.lbl_status_left = self.lbl_record_summary
        self.lbl_status_middle = self.lbl_record_summary
    ...
    def _on_header_section_resized(self, index, old_size, new_size):
        ...
        self._adjust_column_3_width()

    def _adjust_column_3_width(self):
        if getattr(self, "_ignore_min_widths", False):
            return
    ...
    def _select_invoice_by_id(self, invoice_id: int | None):
        """Centralized selection dispatcher to synchronize list row focus, preview, and detail panel."""
        ...
        target_row = -1
        for idx, inv in enumerate(getattr(self, "invoices_list", [])):
        ...
        if target_row != -1:
            ...
            model_index = self.table.model().index(target_row, 0)
    ...
    def _load_invoices(self, reset: bool = True):
        ...
        if reset:
            ...
            if len(self.invoices_list) == 0:
                ...
                if target_row != -1:
                    target_id = self.invoices_list[target_row].get("id")
                    self._select_invoice_by_id(target_id)
                else:
    ...
    def _format_status_count_prefix(self) -> str:
        ...
        total = getattr(self, "incremental_window_total", shown)
        if total > shown:
            return f"当前显示 {shown} / {total} 张｜首屏限量加载"
        return f"当前筛选 {shown} 张"

    def _update_record_summary(self) -> None:
        """Update the combined footer status label with matching count and selection totals."""
        prefix = self._format_status_count_prefix()
        
        # Get selected rows
        selected_indexes = self.table.selectedIndexes()
        # Find unique rows from indexes (since selecting a row selects multiple cells)
        ...
        if not selected_rows:
            ...
            for r in selected_rows:
                if 0 <= r < len(self.invoices_list):
                    rows.append(self.invoices_list[r])
            if not rows:
                text = prefix
            else:
                count, total, has_missing = amount_total(rows)
                suffix = " (部分金额缺失)" if has_missing else ""
                text = f"{prefix}｜已选中 {count} 张｜合计 ¥{total:.2f}{suffix}"
        
        self.lbl_record_summary.setText(text)
        self.lbl_record_summary.setToolTip(text)
    ...
    def _save_invoice_fields(self):
        ...
        try:
            ...
            self._load_invoices()
            if refreshed:
                self._select_invoice_by_id(refreshed.get("id"))
            self._detail_panel.set_dirty_state(False)
                ...
                if status == APPROVED and result["success"] == 1 and len(selected_indexes) == 1:
                    for candidate_row in range(max(0, next_select_row), num_rows):
                        ...
                        if (candidate.get("review_status") or TO_REVIEW) == TO_REVIEW:
                            self._select_invoice_by_id(candidate.get("id"))
                            break
                    else:
                        ...
                        self._select_invoice_by_id(self.invoices_list[next_select_row].get("id"))
                else:
                    if next_select_row < 0:
                        next_select_row = 0
                    elif next_select_row >= num_rows:
